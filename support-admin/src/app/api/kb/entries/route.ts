...
async function parseCreateEntryRequest(request: Request) {
  ...
  try {
    ...
    if (error) {
      return NextResponse.json(
        { ok: false, error: "Не удалось загрузить статьи базы знаний" },
        { status: 500 },
      );
    }
...
export async function POST(request: Request) {
  try {
    ...
    const payload = await parseCreateEntryRequest(request);
    ...
    try {
      ...
      console.error("Failed to create embedding for KB entry:", embeddingError);
